/**
 * 强制拦截的拦截协议（CONTEXT.md：拦截协议）。
 *
 * 重定向规则的**生成**（发端）和 `file=` 参数的**解码**（收端）是同一份契约的
 * 两半，必须住在同一个 module —— 2026-09-21 之前它们散在 rules.ts 和 page.ts，
 * 靠跨文件的隐含默契对齐，协议任何一端想改格式都可能漏改另一端。
 *
 * 有几类文件浏览器判定为「不能内联显示」，直接存盘，页面根本不存在，内容脚本也就
 * 无从注入。实测（Chrome + macOS）落在这一类的只有 `.yaml`/`.yml`
 * （`application/x-yaml`，不在 Blink 可渲染名单）和 `.csv`（在
 * `kUnsupportedTextTypes` 拒绝名单）；`.md` 会内联渲染，`.go`/`.log`/`.toml`
 * 这些系统 MIME 表里没条目的走嗅探变成 `text/plain`，同样内联。
 ...
 *
 * 拦截用 `declarativeNetRequest`：它在导航发生之前就把地址改掉，用户落在的就是
 * 渲染页，当前标签页、前进后退都是正常的一次导航。MV3 的普通扩展没有阻塞式
 * `webRequest`，这是唯一能做到「导航前换掉」的接口。
 *
 * 协议形状（两端都由本 module 拥有）：
 * - 发端：`^file:///.*\\.<ext>$` 命中的整段地址（`\0`）原样写进
 *   `viewer.html?file=` 的 query —— dNR 的替换是静态文本，**不能编码**。
 * - 收端：`fileOf()` 用标准 `URLSearchParams` 取值，`%XX` 会按标准解码。
 *   已知边界（dNR 能力所限，测试里钉死）：文件路径里出现字面 `&` 时 query 在该处
 *   截断 —— 本地文件路径极少这么起名，接受该边界而不是放弃 dNR。
 */
...
/**
 * 发端：规则表。`base` 是扩展自己的地址前缀（`chrome.runtime.getURL("")`）。
 *
