...
- **结构化日志（structured log）**：每条记录都带固定字段，可被机器直接筛选和采集；本仓使用 JSONL 表示，每行一条 JSON。
- **统一日志文件（unified log file）**：全仓 Python 共用的那一个日志文件；靠记录里的来源字段区分是哪个工具写的，而不是每个工具各开一个文件。
- **日志轮转（log rotation）**：日志文件达到大小边界后，当前文件改名为带序号的备份，继续写入新文件；备份数量有限，最旧备份会被删除。
- **日志上下文（log context）**：帮助定位事件的路径、域名、命令和错误信息；不包含密码、cookie 或表单内容。
- **临时日志位置（temporary log location）**：操作系统提供的临时目录；适合近期排障，不保证跨重启保留。
- **私有日志权限（private log permissions）**：日志目录只允许当前用户访问，日志文件和轮转备份只允许当前用户读写。
