...
- **AI 极简输出（省 token）**: 进程由 AI 工具派生（`lib/ai_env.py` 的 `is_ai_shell_env()` 查标记环境变量）时所有输出出口切极简，判定标准只有一条：这段输出喂给模型是不是纯耗 token。成功静默（`ok`/`status(ok|skip)` 不输出，退出码已含此信息），失败/警告单行（`ERROR: <原文>` / `WARN: <原文>`，原文不截断），`timed` 耗时行只在失败时出，`info`/`step`/`rule`/进度条全丢，JSON 走 `lib/ai_env.py` 的 `json_dumps()`（无缩进无空格，字段值一字不丢），fire help 不 `force_terminal`，`--table` 类（archery/browse）强制降级 TSV/JSON。自建 Console 必须 `force_terminal=not is_ai_shell_env()`。见 `.claude/rules/ai-shell-minimal-output.md`
