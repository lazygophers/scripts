...
- The same functions are registered as `[project.scripts]` in `pyproject.toml`, so the whole set installs as real commands: `uvx --from git+https://github.com/lazygophers/scripts <name>` runs any tool without cloning. `bin/<name>` stays the clone-and-run path; both routes execute the identical `lib/cli` function.
