...
- `browse` drives a browser extension from the command line, and the piece that is not visible from a file listing is **why there is a daemon at all**: the browser only ever forks a native messaging host, and the user's CLI is a different process, so the two cannot meet. `lib/browse_daemon.py` is the Unix socket in the middle — one socket, two client roles told apart by a `{"type":"hello","role":"cli"|"native-host"}` handshake. Auth is file permissions only (parent dir 0700, socket 0600, **set at creation** via `umask` before `bind`, because chmod-after-bind leaves a window). Any `browse <module> <action>` auto-spawns the daemon; it exits after 30 idle minutes. The wire shape is WebDriver BiDi's four envelopes, and anything BiDi does not define carries an **`lg:` prefix** (`lg:history.search`, `lg:page.snapshot`) — BiDi §3.3 reserves colon-prefixed module names for exactly this, so the prefix is the marker for "not standard", never decoration. `resolve_method` lets the user drop it (`browse history search`). Two `lg:` methods are deliberately **absent from `lib/cli/browse.py`'s `METHODS`** — `lg:confirm.request` and `lg:context.url` — because they are the daemon asking the extension a question; a CLI able to send them could pop a fake confirmation dialog or probe other tabs. `tests/test_browse_cli.py` asserts that `METHODS` equals the extension's `HANDLERS` minus exactly those two, so adding a handler without a CLI entry (or vice versa) fails. **All confirmation policy is Python-side** (`lib/browse_security.py`): three modes (`silent` / `per_domain` / `always`) in `~/.config/lazygophers/scripts/browse.yaml`, and `--confirm-mode` **can only tighten** — a command line that loosens is refused, since an attacker who can already run commands must not be able to zero the user's policy with one flag. The extension receives "show this dialog", never the policy. High-risk is decided by method name plus one param check: a `js=` **`selector`** evaluates in MAIN world, which is arbitrary page JS wearing a locator's clothes, so `input.click 'js=…'` confirms exactly like `script.evaluate`. `deny_domains` needs a target URL, and `input.*` / `script.*` carry none — the target page is whatever the extension resolves from `context` > `matchUrl` > active tab — so the daemon asks it (`lg:context.url`) before deciding, and **fails closed** when it cannot get an answer. `browse stop` aborts in-flight commands and leaves the daemon up; `browse daemon stop` is the one that exits it. `browse install` / `browse uninstall` register the native host manifest; the code lives in `lib/browse_install.py` rather than under `browser-extension/` precisely so `uvx` users have it, and the manifest's `path` must point at a `browse` that will still exist tomorrow (PATH first, repo `bin/browse` last).
