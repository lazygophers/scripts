...
squash_pr <target> [pr_branch]  # 压当前分支自分叉以来的改动为单 commit → push → 开 PR
```
...
- 🔴 `squash_pr` 的 source 恒为当前分支，不接受指定；第二个位置参数是 PR 分支名（默认 `<当前分支>_pr`）。PR 分支已存在时复用并 force push，保持同一 PR；`--push-only` 只推不开 PR。
