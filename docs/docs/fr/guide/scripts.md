...
- `BATCH_CONCURRENCY` : limite supérieure de parallélisme pour les opérations par lots (`push_*` / `switch_branch` / `sync_master`), par défaut `4`. Exemple : `BATCH_CONCURRENCY=8 push_canary`.
