...
- `BATCH_CONCURRENCY` : верхний предел параллелизма для пакетных операций (`push_*` / `switch_branch` / `sync_master`), по умолчанию `4`. Пример : `BATCH_CONCURRENCY=8 push_canary`.
