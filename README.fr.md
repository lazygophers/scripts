...
>
> **Option globale `--no-say`** : tous les `bin/*` (sauf `n` lui-même) supportent `--no-say` pour couper la voix macOS ; équivalent à `SCRIPTS_NO_SAY=1`. Exemple : `delete_branch --no-say hotfix/x`, `push_canary --no-say`.
>
> **Option `push_*` `--no-check`** : saute les portes de checkwork (pré-vérif branche courante + vérif du résultat fusionné) ; le reste du flux est inchangé. Exemple : `push_canary --no-check`.
